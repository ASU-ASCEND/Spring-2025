...
d3.csv(DATA_PATH, function(d, i) {
    ...
    const ground_speed = +d["MTK3339 Speed"] / 1000 * 2.236936; // +d["TMP117 Temp (C)"];
    ...
    svg.append("g")
        ...
        .call(d3.axisRight(ygs).ticks(height / 40))
        .call(g => g.select(".domain").remove());

    // Create a line generator for internal_temp
    const gs_Line = d3.line()
        .x(d => x(d.date))
        .y(d => ygs(d.ground_speed));
...
function addLegend() {
    ...
    //   { label: "Ozone (64.5–97k ft)", color: "#73A5C6", type: "box" },
      ...
      { label: "Ground Speed (GPS)", color: "blue", type: "line" },
    ];
