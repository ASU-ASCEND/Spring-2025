 ...
 *
 * This function creates a new file entry by first writing a header; then it writes a
 * marker (0xAA) indicating that the file is in progress and reserves one byte for a checksum.
 * It then writes the given data (with a trailing newline) while computing a simple checksum.
 * Once data writing completes, it updates the checksum and changes the marker to 0xBB (complete).
 */
void FlashStorage::atomicStore(String data) {
  ...
  log_flash("AtomicStore complete. Data written with checksum: " + String(checksum));
}

/**
 * @brief Deletes a file by erasing the sectors it occupies.
 *
 * This function locates the specified file in the file_data vector. It then calculates the range
 * of sectors affected (using the sector size) and erases them. For every sector erased, a log is produced.
 *
...
void FlashStorage::removeFile(uint32_t file_number) {
  ...
  if (!found) {
    log_flash("ERROR: File number " + String(file_number) + " not found for deletion.");
    return;
  }
...
/**
 * @brief Checks available flash space and logs warnings when thresholds are reached.
 *
 * Thresholds: 64KB, 16KB, 4KB, and 0 bytes.
 */
void FlashStorage::checkFreeSpaceWarnings() {
  uint32_t remaining = (this->MAX_SIZE > this->address) ? (this->MAX_SIZE - this->address) : 0;
